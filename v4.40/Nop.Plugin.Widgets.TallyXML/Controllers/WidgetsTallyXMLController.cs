...
namespace Nop.Plugin.Widgets.TallyXML.Controllers
{
    ...
    public class WidgetsTallyXMLController : BasePluginController
    {
        ...
        public virtual async Task<IActionResult> AdminView(List<int> orderStatuses = null, List<int> paymentStatuses = null, List<int> shippingStatuses = null)
        {
        ...
        public virtual async Task<IActionResult> OrderList(OrderSearchModel searchModel)
        {
            ...
            var paidOrder = model.Data.Where(x => x.PaymentStatus == "Paid").ToList();
            model.Data = paidOrder;
            return Json(model);
        }
        ...
        public virtual async Task<IActionResult> ExportXmlAll(OrderSearchModel model)
        {
            ...
            var paidOrder = orders.Where(x => x.PaymentStatus == Core.Domain.Payments.PaymentStatus.Paid).ToList();
        ...
        public async void DownloadXML(IList<Order> orders)
        {
            try
            {
                ...
                using (XmlWriter xw = XmlWriter.Create(sb, xws))
                {
                    ...
                    {
                        var customer = _customerService.GetCustomerByIdAsync(m.CustomerId);
                        requestData.Add(new XElement("TALLYMESSAGE", new XAttribute(XNamespace.Xmlns + "UDF", "TallyUDF"),
                                           new XElement("LEDGER", new XAttribute("ACTION", "Create"), new XAttribute("Name", customer.Result.Username),
                                               new XElement("NAME.LIST",
                                                 new XElement("NAME", customer.Result.Username)),
